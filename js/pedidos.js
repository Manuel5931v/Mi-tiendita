...
function renderNuevoPedido() {
  ...
  // Preservar cantidades y foco entre re-renders (fix móvil: oninput perdía foco)
  const qtyPrevia = {};
  ...
  if (bodega.length === 0 && !_bodegaFirebaseVerificado) {
    cargarBodegaDesdeFirebase(() => {
      // Solo re-renderizar si el usuario sigue en la vista nuevo pedido
  ...
  // Restaurar foco en buscador (crítico para móvil)
  if (teniaFocoBuscar) {
